...
class OneClickController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['OneClick'];
			if($model->save())
	...
	{
		$model=new OneClick('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OneClick']))
			$model->attributes=$_GET['OneClick'];
	...
	{
		$model=OneClick::model()->findByPk($id);
		if($model===null)
	...
	public function actionSend()
	{
		...
		$model = new SendMail;
		$model->useHtml = 1;
		$model->sender_email = Yii::app()->params['email'];
		if(isset($_POST['SendMail']))
			$model->attributes = $_POST['SendMail'];
		...
		if(Yii::app()->request->isPostRequest && $model->validate())
		{
			...
			if(!$model->test)
				Yii::app()->request->redirect($this->createUrl('send'));
		}
