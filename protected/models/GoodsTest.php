 ...
 */
/* !!!!!!!!!!!!!!!!!ЭТА МОДЕЛЬ(В ТОМ ЧИСЛЕ И ТАБЛИЦА В БД) НИГДЕ НЕ УЧАСТВУЕТ ОНА НУЖНА ЛИ ЭКСПОРТА ТОВАРОВ НА АЛАНИЮ И БОЛЬШЕ НИ ДЛЯ ЧЕГО!!!!!!!!!!!!!!!!! */
class GoodsTest extends CActiveRecord
{
	...
	**/
	...
	public function getCalculatedPrice($_params = array())
	{
		...
		$cache_id = "getCalculatedPrice".$this->id.$isGuest.$weight.$stone.$size.$forCreateOrder.$withoutRub.$getMetalPrice.$getWorkPrice.$checkStone.$serialkod;
...
// die();
	...
	public function rules()
	{
		...
		return array(
			...
			array('kod, hallmark, goodtype', 'length', 'max'=>13),
	...
	public function getTableSchema()
	{
	  ...
	  $table->foreignKeys['groupkod'] = array('GoodsTest', 'kod');
	...
	public function relations()
	{
		...
		return array(
			...
			'images'=>array(self::HAS_MANY, 'Goodimage', 'good'),
	...
	public static function getGroupItem($group = 0){
		...
		if($group==1){
			...
			return GoodsTest::model()->findAll($criteria);
		}
	...
	public static function getGroupDataProviderItem($groupId, $sort, $pagination)
	{
		/*if(Yii::app()->session->get("admin") == 1)
		{
			...
			$criteria->params = array(':id'=>$groupId);
			$dataprovider = new CActiveDataProvider("GoodsTest",array(
								'criteria'=>$criteria,
								'sort'=>$sort,
								'pagination'=>$pagination
							));
	...
	public function stores($adminPrice=false)
	{
		...
		if((Yii::app()->session->get("admin") == 1)&&($price)){
			foreach ($stores as $data) {
				$item = GoodsTest::model()->findByPk($data->goodkod);
	...
	public function getStoneInStock(){
		...
		$stones = Yii::app()->db->createCommand()
				->select('*')
				->from('goodstore gs')
		...
		foreach ($stones as $key => $data) {
			...
			if(!$data['stonekod']){
				...
				$stonekod = Yii::app()->db->createCommand()
				->select('stonekod')
	...
	public static function getNewGoodStore(){
		$criteria = new CDbCriteria;
		$criteria->join  = 'INNER JOIN goodstore gs ON gs.goodkod = t.kod';
		$criteria->addCondition("publ = 0 AND t.main=1");
		$criteria->group='t.kod';
		$criteria->order = 't.new DESC';
		//$criteria->limit = '5';
		$goods = GoodsTest::model()->findAll($criteria);
		return $goods;
	}
	...
	public function ImageUpdateInWatermark(){
		...
		if($this->images){
			if(!file_exists($this->watermarkDir.$this->images[0]->filename)){
				foreach ($this->images as $data) {
					if(!file_exists($watermarkDir.$data->filename)){
						Yii::app()->ih
						    ->load($_SERVER['DOCUMENT_ROOT'].'/uploads/'.$data->filename)
						    ->resize(1200, 1200)
						    ->watermark($_SERVER['DOCUMENT_ROOT'] . '/images/watermark.png', 40, 40, CImageHandler::CORNER_CENTER, 1)
						    //->thumb(200, 200)
	...
	public function defaultPrice(){
		...
		$price = $this->getCalculatedPrice(array("withoutRub"=>true));
		$defPrice= $price + $price*$koof/100;
		return (int)$defPrice.' руб';
