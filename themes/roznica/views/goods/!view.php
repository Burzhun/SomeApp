...
<script type="text/javascript" src="/themes/roznica/js/jcarousellite_1.0.1.pack.js"></script>
        ...
        <div class="slide2">
                ...
                <div class="clearfix">
                	...
                	<?	//watermark
					if(!file_exists("uploads/watermark/".$model->image)){

						Yii::app()->iwi->load("uploads/".$model->image)->save("uploads/watermark/".$model->image);

						Yii::app()->ih
						    ->load($_SERVER['DOCUMENT_ROOT'].'/uploads/watermark/'.$model->image)
						    ->watermark($_SERVER['DOCUMENT_ROOT'].'/images/watermark.png', 0, 0, CImageHandler::CORNER_CENTER,1)
						    ->save();
					}?>

					<a style='text-align:center;' href="<?=Yii::app()->iwi->load("uploads/watermark/".$model->image)->resize(1600,1600)->cache();?>" class="jqzoom" rel='gal1'  title="triumph" >
              ...
              <ul id="thumblist" class="clearfix mini_zoom" >
						...
						<? foreach ($model->images as $image) { ?>
							...
							<?$img100x100 = Yii::app()->iwi->load("uploads/watermark/".$model->image)->resize(100,100)->cache();
								
							$img380x380 = Yii::app()->iwi->load("uploads/watermark/".$model->image)->resize(380,380)->cache();?>
							<?$img800x800 = Yii::app()->iwi->load("uploads/watermark/".$model->image)->resize(800,800)->cache();?>
							<li>
 	...
 	<??>
        <ul class="items_info">    
        	<li><strong>Артикул: </strong>  <?=$model->article?>      </li>       
          	...
          	<?if(!$stonekod){$stonekod = $model->sizeInStock;}?>
			<li>&nbsp;</li>
...
<?$stonekod = $model->stoneInStock;?>
<?if($stonekod){?>
        <div class="itemHead">Выберите цвет центральной 
вставки (камня)</div>
	...
	<?foreach ($stonekod as $data) {?>
		<?if($prev != $data['stonekod']){?>
			<label data-id="<?=$data['stonekod']?>"><input  type="radio" name='stone'>
				<img src="/uploads/<?=$data['filename'];?>" style="max-width:35px;max-height:35px;" title="<?=$data['stonekod']?>">
			</label>
		<?$prev = $data['stonekod'];}?>
	<?}?>
...
<br> 
		...
		<?foreach ($stonekod as $data) {
			...
			<?if($data['goodsize']!='0.00'){
				...
				<?if($prevSize != $data['stonekod']){
					...
					<label <?=$style?>  data-size="<?=$data['goodsize']?>" data-serialkod="<?=$data['serialkod']?>"><input  type="radio" name='2'><strong><?=$dataSize?></strong></label>
	...
	<form>
		...
		<?echo CHtml::ajaxSubmitButton(
			...
			array(
				'class' => 'to_cart btn toCart itemCart',
				'onClick' => "")
			);?>
	</form>
