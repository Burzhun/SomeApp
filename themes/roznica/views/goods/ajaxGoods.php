<div class="popLeft ajaxitem" data-itemId = '<?=$model->id;?>' data-id=''>◄</div>
<div class="popRight ajaxitem" data-itemId = '<?=$model->id;?>' data-id=''>►</div>
<script>
$(function(){

			          var indexLi  = $(".goods li[data-itemid=<?=$model->id?>]").index();
 
			          	
           prevItem = $(".goods li").eq(indexLi-1).data("itemid");
           nextItem = $(".goods li").eq(indexLi+1).data("itemid");
				$(".popRight").attr("data-id",nextItem);
		          $(".popLeft").attr("data-id",prevItem);
})
</script>
